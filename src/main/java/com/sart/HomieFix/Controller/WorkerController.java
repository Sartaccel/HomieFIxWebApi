...
public class WorkerController {
	...
	public ResponseEntity<List<Worker>> getWorkers() {
		...
		if (workers.isEmpty()) {
			...
			logger.info("Found {} workers", workers.size());
	...
	public ResponseEntity<Worker> getWorkerById(@PathVariable Long id) {
		...
		if (worker == null) {
			logger.warn("Worker with ID {} not found", id); // Use warn for not-found
			return ResponseEntity.notFound().build();
		} else {
			logger.info("Found worker: {}", worker.getName());
	...
	public ResponseEntity<Void> deleteWorker(@PathVariable Long id) {
		...
		logger.info("Worker deleted successfully: {}", id);
		return ResponseEntity.noContent().build();
	}
	...
	public ResponseEntity<Worker> updateWorker(@PathVariable Long id, @RequestParam(required = false) String name,
		...
		try {
			...
			logger.warn("Worker with ID {} not found for update", id); // Log as warning
			return ResponseEntity.notFound().build();
		}
	}
}